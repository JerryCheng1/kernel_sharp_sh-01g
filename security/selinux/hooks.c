...
#ifdef CONFIG_SECURITY_MIYABI_PTRACE_STRICT
	rc = miyabi_ptrace_access_check(child, mode);
	if (rc)
		return rc;
#endif
...
#ifdef CONFIG_SECURITY_MIYABI_PTRACE_STRICT
	rc = miyabi_ptrace_traceme(parent);
	if (rc)
		return rc;
#endif
...
#ifdef CONFIG_SECURITY_MIYABI
	int miyabi_ret = 0;
	miyabi_ret = miyabi_sb_mount(dev_name, path, type, flags, data);
	if (miyabi_ret) return miyabi_ret;
#endif
...
#ifdef CONFIG_SECURITY_MIYABI
	int rc;
	rc = miyabi_dentry_open(file, cred);
	if (rc)
		return rc;
#endif

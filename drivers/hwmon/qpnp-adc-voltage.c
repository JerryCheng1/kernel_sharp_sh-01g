...
#define BEF_THERM_COEFFICIENT	3
#define NOW_THERM_COEFFICIENT	1
#define THERM_INIT_VAL			0x80000000
static int32_t					thermo_ADC_before = THERM_INIT_VAL;
					...
					struct qpnp_vadc_result *result)
{
	...
	while ((vadc->adc->adc_channels[dt_index].channel_num
		...
		dt_index++;
#else  /* CONFIG_BATTERY_SH */
	if( channel == LR_MUX1_BATT_THERM_NO_MOVING_AVE )
	{
	...
	while( ( vadc->adc->adc_channels[dt_index].channel_num != vadc->adc->amux_prop->amux_channel ) &&
	...
	{
		if( vadc->adc->amux_prop->amux_channel == LR_MUX1_BATT_THERM )
		{
			if( likely( thermo_ADC_before != THERM_INIT_VAL ) )
			{
				thermo_ADC_before = ( ( thermo_ADC_before * BEF_THERM_COEFFICIENT ) + ( result->adc_code * NOW_THERM_COEFFICIENT ) ) /
										( BEF_THERM_COEFFICIENT + NOW_THERM_COEFFICIENT );
			}
			else
			{
				thermo_ADC_before = result->adc_code;
			}
			if( channel != LR_MUX1_BATT_THERM_NO_MOVING_AVE )
			{
				result->adc_code = thermo_ADC_before;
			}
		}
		vadc_scale_fn[scale_type].chan(vadc, result->adc_code,
			vadc->adc->adc_prop, vadc->adc->amux_prop->chan_prop, result);
				...
				struct qpnp_vadc_result *result)
{
	...
	if (channel == LR_MUX10_PU1_AMUX_USB_ID_LV) {
		...
		rc = qpnp_vadc_conv_seq_request(vadc, ADC_SEQ_NONE, channel, result);
		if (rc < 0) {
...
#ifdef CONFIG_BATTERY_SH
int32_t qpnp_vadc_read_sh(enum qpnp_vadc_channels channel,
				struct qpnp_vadc_result *result)
{
	int rc = 0;
	...
	rc = qpnp_vadc_read(qpnp_vadc, channel, result);

	return rc;
}
EXPORT_SYMBOL(qpnp_vadc_read_sh);
#endif /* CONFIG_BATTERY_SH */

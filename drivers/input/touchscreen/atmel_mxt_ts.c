...
#ifdef CONFIG_SHTPS_MXT_DEV
	#include <sharp/shtps_dev.h>

	#define SHTPS_VDD_PMIC_PORT_NAME				"8941_l22"
	#define SHTPS_AVDD_PMIC_PORT_NAME				"8941_l10"
#endif /* CONFIG_SHTPS_MXT_DEV */
...
struct mxt_data {
	...
	#ifdef CONFIG_SHTPS_MXT_DEV
		/* Indicates whether device is in lpwg */
		bool in_lpwg;
...
#ifdef CONFIG_SHTPS_MXT_DEV
static int mxt_set_lpwg_mode_on(struct mxt_data *data, struct shtps_lpwg_info *lpwg_info);
static int mxt_set_lpwg_mode_off(struct mxt_data *data);
#endif /* CONFIG_SHTPS_MXT_DEV */
...
static void mxt_proc_t100_message(struct mxt_data *data, u8 *message)
{
	...
	#ifdef CONFIG_SHTPS_MXT_DEV
	{
		...
		dev_dbg(dev,
			"[%u] status:%02X x:%u y:%u peak:%02X h:%02X w:%02X area:%02X amp:%02X vec:%02X\n",
			id,
			status,
			x, y,
			data->t100_aux_peak ? message[data->t100_aux_peak] : 0,
			data->t100_aux_hw ? message[data->t100_aux_hw] : 0,
			data->t100_aux_hw ? message[data->t100_aux_hw+1] : 0,
		...
		tool = ((status & MXT_T100_TYPE_MASK) == MXT_T100_TYPE_STYLUS)? MT_TOOL_PEN : MT_TOOL_FINGER;
		if (data->t100_aux_peak){
			pressure = message[data->t100_aux_peak];
		}
		if (data->t100_aux_hw){
			int major = message[data->t100_aux_hw] > message[data->t100_aux_hw + 1] ?
				message[data->t100_aux_hw] : message[data->t100_aux_hw + 1];
			touch_major = (tool == MT_TOOL_PEN)? MXT_TOUCH_MAJOR_T47_STYLUS : major;
		}
		if (data->t100_aux_area){
			touch_major = (tool == MT_TOOL_PEN)? MXT_TOUCH_MAJOR_T47_STYLUS : message[data->t100_aux_area];
		}
		...
		shtps_mxt_add_input_event(id,
							  status & MXT_T100_DETECT,
							  tool, x, y,
							  (data->t100_aux_area || data->t100_aux_hw), touch_major,
							  (data->t100_aux_ampl || data->t100_aux_peak), pressure,
							  data->t100_aux_vect, orientation);
	}
	#else
		dev_dbg(dev,
...
#ifdef CONFIG_SHTPS_MXT_DEV
static int mxt_proc_t92_messages(struct mxt_data *data, u8 *msg)
{
	struct device *dev = &data->client->dev;
	u8 LSTR, RPTCODE, STRTYPE;
	...
	if(LSTR == 0){
		dev_dbg(dev, "T92 RPTSYM LSTR=%d RPTCODE=%d\n", LSTR, RPTCODE);
	}
	else{
		dev_dbg(dev, "T92 RPTSYM LSTR=%d STRTYPE=%d\n", LSTR, STRTYPE);
	}
	shtps_mxt_lpwg_wakeup();
...
static irqreturn_t mxt_process_messages_t44(struct mxt_data *data)
{
	...
	if (data->update_input) {
		#ifdef CONFIG_SHTPS_MXT_DEV
			shtps_mxt_input_sync(data->input_dev);
		#endif /* CONFIG_SHTPS_MXT_DEV */
...
static irqreturn_t mxt_interrupt(int irq, void *dev_id)
{
	...
	irqreturn_t irqret;
	int lpwg_wakelock_on = 0;

	dev_dbg(&data->client->dev, "%s: irq=%d\n", __func__, irq);

	shtps_mxt_wake_lock_idle();

	if (data->in_bootloader) {
		/* bootloader state transition completion */
		complete(&data->bl_completion);
		dev_dbg(&data->client->dev, "%s: bootloader state transition completed\n", __func__);
		irqret = IRQ_HANDLED;
		goto END;
	}
	...
	if(data->in_lpwg == true){
		shtps_mxt_lpwg_wakelock(1);
		lpwg_wakelock_on = 1;
	}

	if (data->T44_address) {
		irqret = mxt_process_messages_t44(data);
	} else {
		irqret = mxt_process_messages(data);
	}

END:
	if(lpwg_wakelock_on == 1){
		shtps_mxt_lpwg_wakelock(0);
	}
	shtps_mxt_wake_unlock_idle();
...
static int mxt_acquire_irq(struct mxt_data *data)
{
	...
	#ifdef CONFIG_SHTPS_MXT_DEV
		if(data->is_irq_enable == true){
			return 0;
		}

		enable_irq(data->irq);
		enable_irq_wake(data->irq);
		data->is_irq_enable = true;

		dev_dbg(&data->client->dev, "enable_irq()\n");
	#else
		enable_irq(data->irq);
	#endif /* CONFIG_SHTPS_MXT_DEV */
...
#ifdef CONFIG_SHTPS_MXT_DEV
static int mxt_release_irq(struct mxt_data *data)
{
	if(data->is_irq_enable == false){
		return 0;
	}

	disable_irq_wake(data->irq);
	disable_irq(data->irq);
	data->is_irq_enable = false;

	dev_dbg(&data->client->dev, "disable_irq()\n");
...
static void mxt_free_input_device(struct mxt_data *data)
{
	if (data->input_dev) {
		...
		data->input_dev = NULL;
		#ifdef CONFIG_SHTPS_MXT_DEV
			shtps_mxt_notify_free_input_dev();
		#endif /* CONFIG_SHTPS_MXT_DEV */
	}
}
...
static void mxt_regulator_enable(struct mxt_data *data)
{
	...
	ret = -1;
	enabled = regulator_is_enabled(data->reg_vdd);

	if (enabled){
		ret = regulator_set_mode(data->reg_vdd, REGULATOR_MODE_NORMAL);
	}else{
		WARN_ON(!enabled);
	}

	if(ret != 0) {
		dev_err(dev, "regulator_set_mode(reg_vdd, %d) fail, ret=%d\n", REGULATOR_MODE_NORMAL, ret);
	}
	else{
		dev_dbg(dev, "regulator_set_mode(reg_vdd, %d) success.\n", REGULATOR_MODE_NORMAL);
	}

	enabled = regulator_is_enabled(data->reg_avdd);

	ret = -1;
	if (enabled){
		ret = regulator_set_mode(data->reg_avdd, REGULATOR_MODE_NORMAL);
	}else{
		WARN_ON(!enabled);
	}

	if(ret != 0) {
		dev_err(dev, "regulator_set_mode(reg_avdd, %d) fail, ret=%d\n", REGULATOR_MODE_NORMAL, ret);
	}
	else{
		dev_dbg(dev, "regulator_set_mode(reg_avdd, %d) success.\n", REGULATOR_MODE_NORMAL);
	}
...
static void mxt_regulator_disable(struct mxt_data *data)
{
	...
	ret = -1;
	if (enabled){
		ret = regulator_set_mode(data->reg_vdd, REGULATOR_MODE_IDLE);
	}else{
		WARN_ON(!enabled);
	}

	if(ret != 0) {
		dev_err(dev, "regulator_set_mode(reg_vdd, %d) fail, ret=%d\n", REGULATOR_MODE_IDLE, ret);
	}
	else{
		dev_dbg(dev, "regulator_set_mode(reg_vdd, %d) success.\n", REGULATOR_MODE_IDLE);
	}
	...
	ret = -1;
	if (enabled){
		ret = regulator_set_mode(data->reg_avdd, REGULATOR_MODE_IDLE);
	}else{
		WARN_ON(!enabled);
	}
...
static void mxt_probe_regulators(struct mxt_data *data)
{
	...
	data->reg_vdd = regulator_get(dev, SHTPS_VDD_PMIC_PORT_NAME);
	if (IS_ERR(data->reg_vdd)) {
		dev_err(dev, "Unable to get %s regulator\n", SHTPS_VDD_PMIC_PORT_NAME);
		goto fail;
	}

	data->reg_avdd = regulator_get(dev, SHTPS_AVDD_PMIC_PORT_NAME);
	if (IS_ERR(data->reg_avdd)) {
		dev_err(dev, "Unable to get %s regulator\n", SHTPS_AVDD_PMIC_PORT_NAME);
		goto fail_release;
	}
...
static int mxt_read_t100_config(struct mxt_data *data)
{
...
#ifdef CONFIG_SHTPS_MXT_DEV
	aux_size_max = data->T5_msg_size - aux;
	aux_offset = 0;
	data->t100_aux_vect = 0;
	data->t100_aux_ampl = 0;
	data->t100_aux_area = 0;
	data->t100_aux_hw = 0;
	data->t100_aux_peak = 0;
	data->t100_aux_areahw = 0;

	if(tchaux & MXT_T100_TCHAUX_VECT){
		data->t100_aux_vect = aux + aux_offset;
		aux_offset++;
	}

	if(tchaux & MXT_T100_TCHAUX_AMPL){
		data->t100_aux_ampl = aux + aux_offset;
		aux_offset++;
	}

	if(tchaux & MXT_T100_TCHAUX_AREA){
		data->t100_aux_area = aux + aux_offset;
		aux_offset++;
	}

	if(tchaux & MXT_T100_TCHAUX_HW){
		if((aux_offset + 1) < aux_size_max){
			data->t100_aux_hw = aux + aux_offset;
		}
		aux_offset += 2;
	}

	if(tchaux & MXT_T100_TCHAUX_PEAK){
		if(aux_offset < aux_size_max){
			data->t100_aux_peak = aux + aux_offset;
		}
		aux_offset++;
	}
	...
	if(tchaux & MXT_T100_TCHAUX_AREAHW){
		if((aux_offset + 1) < aux_size_max){
			data->t100_aux_areahw = aux + aux_offset;
		}
		aux_offset += 2;
	}
#else
...
static int mxt_initialize_t100_input_device(struct mxt_data *data)
{
...
#ifdef CONFIG_SHTPS_MXT_DEV
	if ( (data->t100_aux_area) || (data->t100_aux_hw) ) {
		input_set_abs_params(input_dev, ABS_MT_TOUCH_MAJOR,
				     0, MXT_MAX_AREA, 0, 0);
	}

	if ( (data->t100_aux_ampl) || (data->t100_aux_peak) ) {
	...
	#ifdef CONFIG_SHTPS_MXT_DEV
		shtps_mxt_notify_make_input_dev(data->input_dev);
	#endif /* CONFIG_SHTPS_MXT_DEV */
	...
	size_t count)
{
	...
	return ret == 0 ? count : ret;
		...
		static DEVICE_ATTR(fw_version, (S_IRUGO | S_IWUGO), mxt_fw_version_show, NULL);
		static DEVICE_ATTR(hw_version, (S_IRUGO | S_IWUGO), mxt_hw_version_show, NULL);
		static DEVICE_ATTR(object, (S_IRUGO | S_IWUGO), mxt_object_show, NULL);
		static DEVICE_ATTR(update_fw, (S_IRUGO | S_IWUGO), NULL, mxt_update_fw_store);
		static DEVICE_ATTR(update_cfg, (S_IRUGO | S_IWUGO), NULL, mxt_update_cfg_store);
		static DEVICE_ATTR(debug_v2_enable, (S_IRUGO | S_IWUGO), NULL,
				   mxt_debug_v2_enable_store);
		static DEVICE_ATTR(debug_notify, (S_IRUGO | S_IWUGO), mxt_debug_notify_show, NULL);
		static DEVICE_ATTR(debug_enable, (S_IRUGO | S_IWUGO), mxt_debug_enable_show,
...
static void mxt_reset_slots(struct mxt_data *data)
{
	struct input_dev *input_dev = data->input_dev;
#ifdef CONFIG_SHTPS_MXT_DEV
	shtps_mxt_input_force_touchup(input_dev);
#else
...
static void mxt_start(struct mxt_data *data)
{
	...
	shtps_mxt_notify_devstate(SHTPS_DEV_STATE_ACTIVE);
#else
	if (data->use_regulator) {
		enable_irq(data->irq);
...
static void mxt_stop(struct mxt_data *data)
{
	...
	shtps_mxt_notify_devstate(SHTPS_DEV_STATE_SLEEP);
#else
	disable_irq(data->irq);
...
#ifdef CONFIG_SHTPS_MXT_DEV
static void mxt_lpwg(struct mxt_data *data, int on, struct shtps_lpwg_info *lpwg_info)
{
	int ret;
	if (data->in_bootloader || data->updating_config) {
		dev_info(&data->client->dev, "current state is not change to lpwg.\n");
		return;
	}
	...
	if(on == 1){
		/* to lpwg mode */
		if(lpwg_info == NULL){
			dev_info(&data->client->dev, "parameter is few. not change to lpwg.\n");
			return;
		}
		...
		ret = mxt_set_lpwg_mode_on(data, lpwg_info);
		if(ret == 0){
			mxt_acquire_irq(data);
			shtps_mxt_notify_devstate(SHTPS_DEV_STATE_LPWG);
		}
		else{
			dev_info(&data->client->dev, "error occurs(%d). not change to lpwg.\n", ret);
			/* to sleep mode setting */
			on = 0;
		}
	}
...
#ifdef CONFIG_OF
	data->irq = irq_of_parse_and_map(data->client->dev.of_node, 0);
	data->pdata->gpio_reset = of_get_named_gpio(data->client->dev.of_node, "atmel,reset-gpio", 0);
#endif
...
static int mxt_set_lpwg_mode_on(struct mxt_data *data, struct shtps_lpwg_info *lpwg_info)
{
	...
	buf[0] &= ~0x02;
	error = __mxt_write_reg(data->client, object->start_address + 0,
				1, buf);
	if (error)
		return error;

	/* set T7: IDLEACQINT, ACTVACQINT */
	buf[0] = lpwg_info->idleacqint;
	buf[1] = lpwg_info->actvacqint;
	error = __mxt_write_reg(data->client, data->T7_address + 0,
				2, buf);
	if (error)
		return error;

	dev_dbg(&client->dev, "Set T7 ACTV:%d IDLE:%d\n", buf[1], buf[0]);

	if((lpwg_info->symdata_size > 0) && (lpwg_info->symdata != NULL)){
		/* set T92: SYMDATA[] */
		object = mxt_get_object(data, MXT_PROCI_SYMBOLGESTUREPROCESSOR_T92);
		if (!object)
			return -EINVAL;

		error = __mxt_write_reg(data->client, object->start_address + 10,
				lpwg_info->symdata_size, lpwg_info->symdata);
		if (error)
			return error;
	}

	/* T92: RPTEN and ENABLE set 1 */
	object = mxt_get_object(data, MXT_PROCI_SYMBOLGESTUREPROCESSOR_T92);
	if (!object)
		return -EINVAL;
...
EXPORT_SYMBOL(mxt_power_enable);
...
EXPORT_SYMBOL(mxt_power_disable);
...
int mxt_hard_reset(struct device *dev)
{
	...
	pre_irq_enable = data->is_irq_enable;
	if(pre_irq_enable == false){
		mxt_acquire_irq(data);
	}

	dev_info(dev, "set reset pin LOW\n");
	gpio_set_value(data->pdata->gpio_reset, 0);
	msleep(MXT_REGULATOR_DELAY);
	dev_info(dev, "set reset pin HIGH\n");
	gpio_set_value(data->pdata->gpio_reset, 1);
	msleep(MXT_CHG_DELAY);
...
EXPORT_SYMBOL(mxt_set_sleep);

int mxt_set_lpwg(struct device *dev, int on, struct shtps_lpwg_info *lpwg_info)
{
	struct mxt_data *data = dev_get_drvdata(dev);
	struct input_dev *input_dev;
	...
	mxt_lpwg(data, on, lpwg_info);

	mutex_unlock(&input_dev->mutex);

	return 0;
}
EXPORT_SYMBOL(mxt_set_lpwg);
...
#ifdef CONFIG_SHTPS_MXT_DEV
	/* reset here because reset does not be performed when regulators probe failed */
	error = mxt_hard_reset(&client->dev);
#endif /* CONFIG_SHTPS_MXT_DEV */
...
#ifdef CONFIG_SHTPS_MXT_DEV
	shtps_mxt_cust_initialize(&client->dev);
#endif /* CONFIG_SHTPS_MXT_DEV */

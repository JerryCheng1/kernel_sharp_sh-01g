...
#define QPNP_CHG_ENABLE_PARA_CONN
...
#ifndef QPNP_CHG_ENABLE_PARA_CONN
static irqreturn_t
...
#endif /* QPNP_CHG_ENABLE_PARA_CONN */
...
#ifdef CONFIG_BATTERY_SH
#define MIN_DELTA_MV_TO_INCREASE_VDD_MAX	9
#define MAX_DELTA_VDD_MAX_MV			80
#define VDD_MAX_CENTER_OFFSET			0
#else
...
int qpnp_chg_battery_transistor_switch(int enable)
{
	...
	if (charge_en != -1)
	{
		if (prev_charge_en == 0 && charge_en == 1)
		{
			the_chip->delta_vddmax_mv = 0;
		}
		prev_charge_en = charge_en;
...
int qpnp_chg_charger_vddmax_set(int voltage)
{
	...
	return qpnp_chg_vddmax_and_trim_set(the_chip, the_chip->max_voltage_mv, the_chip->delta_vddmax_mv);
}
...
int qpnp_chg_get_iusb_trim(void)
{
	if (!the_chip)
	{
		pr_err("qpnp charger is not initialized\n");
	...
	return qpnp_chg_iusb_trim_get(the_chip);
}
EXPORT_SYMBOL_GPL(qpnp_chg_get_iusb_trim);

int qpnp_chg_get_idc_trim(void)
{
	if (!the_chip)
	{
		pr_err("qpnp charger is not initialized\n");
	...
	return qpnp_chg_idc_trim_get(the_chip);
}
EXPORT_SYMBOL_GPL(qpnp_chg_get_idc_trim);

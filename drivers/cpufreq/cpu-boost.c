...
#ifdef CONFIG_SHSYS_CUST
static unsigned int migration_sync_freq = 1190400;
module_param(migration_sync_freq, uint, 0644);
#endif /* CONFIG_SHSYS_CUST */
...
#ifdef CONFIG_SHSYS_CUST
		if (ret){
			pr_debug("boost_mig_sync_thread : Failure get src_cpu(cpu%d) policy\n",src_cpu);
			src_policy.cur = migration_sync_freq;
		}
#else
		if (ret)
			continue;

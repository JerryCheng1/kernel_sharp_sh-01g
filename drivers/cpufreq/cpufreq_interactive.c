...
extern bool kgsl_is_working(void);
extern int msm_routing_get_is_music_play(void);

#define DEF_FREQ_STEP					(33)
#define DEF_FREQ_STEP_HIGH				(5)
#define DEF_FREQUENCY_THRESHOLD_ONDEMAND_STEPHIGH		(99)
...
static unsigned int up_threshold_freq_step_high = DEF_FREQUENCY_THRESHOLD_ONDEMAND_STEPHIGH;
static unsigned int up_threshold_over_freq_step_high = DEF_FREQUENCY_THRESHOLD_OVER_ONDEMAND_STEPHIGH;
...
#ifdef CONFIG_SHSYS_CUST
		if (pcpu->target_freq < interactive_threshold)
			new_freq = interactive_step_freq;
		else {
			int inc = (pcpu->policy->cpuinfo.max_freq * freq_step) / 100;
			new_freq = pcpu->target_freq + inc;
		}
		if (pcpu->target_freq < interactive_step_high_freq && new_freq > interactive_step_high_freq) {
			if(cpu_load > up_threshold_freq_step_high){
				new_freq = interactive_step_high_freq;
			}
			else{
				new_freq = pcpu->target_freq;
			}
		}else if (pcpu->target_freq >= interactive_step_high_freq){
			if(cpu_load > up_threshold_over_freq_step_high){
				int inc = (pcpu->policy->cpuinfo.max_freq * freq_step_high) / 100;
				new_freq = pcpu->target_freq + inc;
			}
			else{
				new_freq = pcpu->target_freq;
...
#ifdef CONFIG_SHSYS_CUST
	if(mdss_mdp_get_mdp_clk_ena(&mdp_clock_enable) != 0){
		pr_debug("cpu%d : mdss_mdp_get_mdp_clk_ena error \n", smp_processor_id());
	}
	if (mdp_clock_enable == 0 && kgsl_is_working() == false && msm_routing_get_is_music_play() == 0) {
		pr_debug("cpu%d : mdp_clk = %d gpu_clk = %d is_music_play = %d check clock up elapsed time \n", smp_processor_id(), mdp_clock_enable, kgsl_is_working(),msm_routing_get_is_music_play());
		if (pcpu->target_freq >= hispeed_freq &&
		    new_freq > pcpu->target_freq &&
...
#ifdef CONFIG_SHSYS_CUST
	if (mdp_clock_enable == 1 || kgsl_is_working() == true || msm_routing_get_is_music_play() != 0) {
		pr_debug("cpu%d : mdp_clk = %d gpu_clk = %d is_music_play = %d check clock down elapsed time \n", smp_processor_id(), mdp_clock_enable, kgsl_is_working(),msm_routing_get_is_music_play());
...
#ifdef CONFIG_SHSYS_CUST
	pr_info("interactive boost reject request = %lu \n",val);
#else
	boost_val = val;
...
#ifdef CONFIG_SHSYS_CUST
	pr_info("interactive boostpulse reject request = %lu \n",val);
#else
...
static struct global_attr freq_step_attr =
		__ATTR(freq_step, 0644,
		show_freq_step,
				store_freq_step);

static ssize_t show_freq_step_high(struct kobject *kobj,
			struct attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%u\n", freq_step_high);
}

static ssize_t store_freq_step_high(struct kobject *kobj,
			struct attribute *attr, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static struct global_attr freq_step_high_attr =
		__ATTR(freq_step_high, 0644,
		show_freq_step_high,
				store_freq_step_high);

static ssize_t show_up_threshold_freq_step_high(struct kobject *kobj,
			struct attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%u\n", up_threshold_freq_step_high);
}

static ssize_t store_up_threshold_freq_step_high(struct kobject *kobj,
			struct attribute *attr, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	if (ret < 0)
		return ret;
	up_threshold_freq_step_high = val;
	return count;
}

static struct global_attr up_threshold_freq_step_high_attr =
		__ATTR(up_threshold_freq_step_high, 0644,
		show_up_threshold_freq_step_high,
				store_up_threshold_freq_step_high);

static ssize_t show_up_threshold_over_freq_step_high(struct kobject *kobj,
			struct attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%u\n", up_threshold_over_freq_step_high);
}

static ssize_t store_up_threshold_over_freq_step_high(struct kobject *kobj,
			struct attribute *attr, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	if (ret < 0)
		return ret;
	up_threshold_over_freq_step_high = val;
	return count;
}

static struct global_attr up_threshold_over_freq_step_high_attr =
		__ATTR(up_threshold_over_freq_step_high, 0644,
		show_up_threshold_over_freq_step_high,
				store_up_threshold_over_freq_step_high);

static ssize_t show_interactive_threshold(struct kobject *kobj,
			struct attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%u\n", interactive_threshold);
}

static ssize_t store_interactive_threshold(struct kobject *kobj,
			struct attribute *attr, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	if (ret < 0)
		return ret;
	interactive_threshold = val;
	return count;
}

static struct global_attr interactive_threshold_attr =
		__ATTR(interactive_threshold, 0644,
		show_interactive_threshold,
				store_interactive_threshold);

static ssize_t show_interactive_step_freq(struct kobject *kobj,
			struct attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%u\n", interactive_step_freq);
}

static ssize_t store_interactive_step_freq(struct kobject *kobj,
			struct attribute *attr, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	if (ret < 0)
		return ret;
	interactive_step_freq = val;
	return count;
}

static struct global_attr interactive_step_freq_attr =
		__ATTR(interactive_step_freq, 0644,
		show_interactive_step_freq,
				store_interactive_step_freq);

static ssize_t show_interactive_step_high_freq(struct kobject *kobj,
			struct attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%u\n", interactive_step_high_freq);
}

static ssize_t store_interactive_step_high_freq(struct kobject *kobj,
			struct attribute *attr, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	if (ret < 0)
		return ret;
	interactive_step_high_freq = val;
	return count;
}

static struct global_attr interactive_step_high_freq_attr =
		__ATTR(interactive_step_high_freq, 0644,
		show_interactive_step_high_freq,
				store_interactive_step_high_freq);
...
#ifdef CONFIG_SHSYS_CUST
	&freq_step_attr.attr,
	&freq_step_high_attr.attr,
	&up_threshold_freq_step_high_attr.attr,
	&up_threshold_over_freq_step_high_attr.attr,

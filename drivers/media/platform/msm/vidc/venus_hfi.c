...
static inline int venus_hfi_clk_gating_off(struct venus_hfi_device *device)
{
	...
	if (wake_lock_active(&device->video_wake_lock)) {
		wake_unlock(&device->video_wake_lock);
	}
...
static inline void venus_hfi_clk_gating_on(struct venus_hfi_device *device)
{
	...
	if(!wake_lock_active(&device->video_wake_lock)) {
		wake_lock(&device->video_wake_lock);
	}
			...
			hfi_cmd_response_callback callback)
{
	...
	wake_lock_init(&hdevice->video_wake_lock, WAKE_LOCK_SUSPEND, "video_suspend");
	...
	if (device) {
		...
		list_for_each_entry_safe(close, tmp, &hal_ctxt.dev_head, list) {
			if (close->hal_data->irq == dev->hal_data->irq) {
				...
				wake_lock_destroy(&dev->video_wake_lock);

...
/* SHLOCAL_CAMERA_DRIVERS-> */
	...
	CDBG("%s rev=0x%0x\n", __func__, rev);
	if(rev == 0x00){
		unsigned gpio_config;
		gpio_config = GPIO_CFG( 21, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA );
		
		rc = gpio_tlmm_config( gpio_config, GPIO_CFG_ENABLE );
		CDBG("%s:%d rc %d\n", __func__, __LINE__, rc);
		if( rc < 0 ) {
			cci_dev->ref_count--;
			CDBG("%s: gpio_tlmm_config 21 failed\n", __func__);
			goto request_gpio_failed;
		}
		gpio_config = GPIO_CFG( 22, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA );
		
		rc = gpio_tlmm_config( gpio_config, GPIO_CFG_ENABLE );
		CDBG("%s:%d rc %d\n", __func__, __LINE__, rc);
		if( rc < 0 ) {
			cci_dev->ref_count--;
			CDBG("%s: gpio_tlmm_config 22 failed\n", __func__);
			goto request_gpio_failed;
		}
	}
#endif

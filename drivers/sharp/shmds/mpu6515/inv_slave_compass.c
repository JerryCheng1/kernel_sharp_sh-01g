...
static int diag_show_raw_compass(struct inv_mpu_state *st,char *buf){
	int result;
	...
	if (0 == st-st->sensor[SENSOR_COMPASS].on){
		return -EPERM;
	}
...
static int diag_show_compass_self_test(struct inv_mpu_state *st,char *buf){
	int result;
	int bias[3];
	unsigned char *sens;
	...
	if (0 == st->sensor[SENSOR_COMPASS].on){
		return -EPERM;
	}
...
/* SHMDS_ID_0201_01 add S */
	.diag_read_data = diag_show_raw_compass,
	.diag_self_test = diag_show_compass_self_test,
/* SHMDS_ID_0201_01 add E */

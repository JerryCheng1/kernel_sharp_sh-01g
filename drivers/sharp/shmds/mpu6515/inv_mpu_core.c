...
#endif
/* SHMDS_ID_0207_01 add S */
#include <sharp/shmds_driver.h>
/* SHMDS_ID_0207_01 add E */
/* SHMDS_ID_0104_01 add S */
#include <linux/gpio.h>
/* SHMDS_ID_0104_01 add E */

/* SHMDS_ID_1204_01 add S */
#include <linux/pm_qos.h>
#include <mach/cpuidle.h>

/* SHMDS_ID_0111_01 add S */
#ifdef CONFIG_ANDROID_ENGINEERING
static int dbg_log_level_inv_mpu_core = 0;
module_param(dbg_log_level_inv_mpu_core,  int, 0600);
#define SHMDS_DBG_LOG_LOW(fmt, args...)  \
	if (dbg_log_level_inv_mpu_core > 1) { \
		printk("[SHMDS][%s] " fmt, __func__, ## args); \
	}

#define SHMDS_DBG_LOG_HIGH(fmt, args...)  \
	if (dbg_log_level_inv_mpu_core > 0) { \
		printk("[SHMDS][%s] " fmt, __func__, ## args); \
	}
#else
...
/* SHMDS_ID_0207_01 add S */
atomic_t report_flg = ATOMIC_INIT(SMD_REPORT);
struct timer_list restart_smd_timer;
static struct mutex smd_timerlock;
atomic_t smd_timer_kind_flg = ATOMIC_INIT(SMD_TIMER_ADD);

void smd_delete_timer(void)
{
    mutex_lock(&smd_timerlock);
    del_timer_sync(&restart_smd_timer);
    atomic_set( &smd_timer_kind_flg, SMD_TIMER_ADD );
    mutex_unlock(&smd_timerlock);
}

void smd_start_timer(void)
{
    if (atomic_read( &smd_timer_kind_flg ) == SMD_TIMER_ADD )
    {
		restart_smd_timer.expires = jiffies + msecs_to_jiffies(1000);
		add_timer(&restart_smd_timer);
		atomic_set( &smd_timer_kind_flg, SMD_TIMER_MOD );
    }
    else
    {
        mod_timer(&restart_smd_timer,jiffies + msecs_to_jiffies(500));
        atomic_set( &smd_timer_kind_flg, SMD_TIMER_MOD );
	}
}

int shmds_api_stop_pedometer_func(int type)
{
	if(type == SHMDS_STOP_PED_TYPE_VIB) {
		smd_delete_timer();
		atomic_set(&report_flg, SMD_NO_REPORT);
	}
	return 0;
}
int shmds_api_restart_pedometer_func(int type)
{
	if(type == SHMDS_STOP_PED_TYPE_VIB)
	{
		mutex_lock(&smd_timerlock);
...
void restart_ped_timer_handler(unsigned long data)
{
		atomic_set(&report_flg, SMD_REPORT);
}
/* SHMDS_ID_0207_01 add E */
...
/* SHMDS_ID_1204_01 add E */

s64 get_time_ns(void)
{
	struct timespec ts;

/* SHMDS_ID_1709_01 mod A */
#ifdef INV_KERNEL_3_10
	get_monotonic_boottime(&ts);
#else
	ts = ktime_to_timespec(alarm_get_elapsed_realtime());
#endif
/* SHMDS_ID_1709_01 mod E */
...
/* SHMDS_ID_0201_01 add S */
	reg->raw_gyro		= REG_RAW_GYRO;
/* SHMDS_ID_0201_01 add E */
...
/* SHMDS_ID_1707_01 mod S */
//		usleep(SENSOR_UP_UTIME);
		usleep(100000);
/* SHMDS_ID_1707_01 mod E */
...
/* SHMDS_ID_1202_01 mod S */
	if (power_on){
//		msleep(REG_UP_TIME);
		usleep(REG_UP_UTIME);
	}
/* SHMDS_ID_1202_01 mod E */
...
static int inv_init_config(struct iio_dev *indio_dev)
{
	...
	if (INV_ITG3500 != st->chip_type) {
		...
		st->ped.max_step_buffer_time = INIT_PEDOMETER_MAX_STEP_BUFFER_TIME;
		st->ped.threshold = INIT_PEDOMETER_THRESHOLD;
		st->ped.min_up_time = INIT_PEDOMETER_MIN_UP_TIME;
		st->ped.max_up_time = INIT_PEDOMETER_MAX_UP_TIME;
...
#ifdef SHMDS_DETECT
    fifo_rate = shmds_check_mot_fifo_rate(st, fifo_rate, 0);
#endif /* SHMDS_DETECT */
/* SHMDS_ID_1101_01 add E */
/* SHMDS_ID_2001_01 mod S */
...
/* SHMDS_ID_0111_01 add S */
	SHMDS_DBG_LOG_HIGH("case=%d data=%d\n", (int)this_attr->address, data);
/* SHMDS_ID_0111_01 add E */
...
/* SHMDS_ID_1101_01 add S */
#ifdef SHMDS_DETECT
		data = shmds_check_mot_fifo_rate(st, data, 0);
#endif /* SHMDS_DETECT */
/* SHMDS_ID_1101_01 add E */
...
/* for adb debug */
    ...
    case ATTR_SHMDS_THS_VALUE:
        result = shmds_ths_value_store(data);
        break;
    case ATTR_SHMDS_DURATION_VALUE:
        result = shmds_duration_value_store(data);
        break;
    case ATTR_SHMDS_BUF_SIZE_TEMP:
        result = shmds_buf_size_temp_store(data);
        break;
    case ATTR_SHMDS_MD_THRESH:
        result = shmds_md_thresh_store(data);
        break;
    case ATTR_SHMDS_MG_TO_LSB:
        result = shmds_mg_to_LSB_store(data);
        break;
    case ATTR_SHMDS_MD_THRESH_VALUE:
        result = shmds_md_thresh_value_store(data);
        break;
...
/* SHMDS_ID_0111_01 add E */
	...
	case ATTR_DMP_PEDOMETER_MAX_STEP_BUFFER_TIME:
	{
		...
		if (result)
			goto dmp_mem_store_fail;
		st->ped.max_step_buffer_time = data;
		break;
	}
...
/* SHMDS_ID_0111_01 add S */
	SHMDS_DBG_LOG_HIGH("case=%d data=%lld\n", (int)this_attr->address, tmp);
/* SHMDS_ID_0111_01 add E */
	mutex_unlock(&indio_dev->mlock);
	if (result)
		return -EINVAL;
...
/* for adb debug */
#if defined(SHMDS_DETECT) && defined(SHMDS_ADB_FLAG)
    case ATTR_SHMDS_COUNT:
	    return shmds_count_show(buf);
    case ATTR_SHMDS_THS_VALUE:
	    return shmds_ths_value_show(buf);
    case ATTR_SHMDS_DURATION_VALUE:
	    return shmds_duration_value_show(buf);
    case ATTR_SHMDS_BUF_SIZE_TEMP:
	    return shmds_buf_size_temp_show(buf);
    case ATTR_SHMDS_MD_THRESH:
	    return shmds_md_thresh_show(buf);
    case ATTR_SHMDS_MG_TO_LSB:
	    return shmds_mg_to_LSB_show(buf);
    case ATTR_SHMDS_MD_THRESH_VALUE:
	    return shmds_md_thresh_value_show(buf);
#endif /* SHMDS_DETECT, SHMDS_ADB_FLAG */
/* SHMDS_ID_1101_01 add E */
#ifdef CONFIG_INV_TESTING
	case ATTR_REG_WRITE:
...
/* SHMDS_ID_1101_01 add S */
#ifdef SHMDS_DETECT
		data = shmds_check_mot_fifo_rate(st, data, 1);
#endif /* SHMDS_DETECT */
/* SHMDS_ID_1101_01 add E */
...
static int diag_store_power_state(struct inv_mpu_state *st,int power_state){
	struct inv_reg_map_s *reg;
	int result;
	unsigned char data;
	...
	if (power_state) {
		usleep(5000);
		data = 0;
		if (0 == st->chip_config.accel_enable){
			data |= BIT_PWR_ACCEL_STBY;
		}
...
static int diag_store_gyro_enable(struct inv_mpu_state *st,const char *buf, size_t count){
	bool en;
	int result;
	unsigned long data;
	...
	result = inv_switch_engine(st, en, BIT_PWR_GYRO_STBY);
	if (result){
		return result;
	}
...
static int diag_store_accel_enable(struct inv_mpu_state *st,const char *buf, size_t count){
	bool en;
	int result;
	unsigned long data;
	...
	if (en == st->chip_config.accel_enable){
		return count;
	}
	
	result = inv_switch_engine(st, en, BIT_PWR_ACCEL_STBY);
	if (result){
		return result;
	}
	st->chip_config.accel_enable = en;

	return count;
}

static int diag_store_compass_enable(struct inv_mpu_state *st,const char *buf, size_t count){
	bool en;
	int result;
	unsigned long data;
	...
	if (en == st->sensor[SENSOR_COMPASS].on){
		return count;
	}
	
	st->sensor[SENSOR_COMPASS].on = en;

	return count;
}

static int diag_store_reference_register(struct inv_mpu_state *st,const char *buf, size_t count){
	int result = 0;
...
static int diag_store_rw_register(struct inv_mpu_state *st,const char *buf, size_t count){
	int result = 0;
...
static int diag_show_raw_gyro(struct inv_mpu_state *st,char *buf){
	struct inv_reg_map_s *reg;
	int result;
...
static int diag_show_raw_accel(struct inv_mpu_state *st,char *buf){
	struct inv_reg_map_s *reg;
	int result;
	unsigned char data[BYTES_PER_SENSOR];
...
static int diag_show_power_state(struct inv_mpu_state *st,char *buf){
	if (st->chip_config.is_asleep){
...
static int diag_show_gyro_enable(struct inv_mpu_state *st,char *buf){
	return sprintf(buf, "%d\n", st->chip_config.gyro_enable);
}

static int diag_show_accel_enable(struct inv_mpu_state *st,char *buf){
	return sprintf(buf, "%d\n", st->chip_config.accel_enable);
}

static int diag_show_compass_enable(struct inv_mpu_state *st,char *buf){
	return sprintf(buf, "%d\n", st->sensor[SENSOR_COMPASS].on);
}

static int diag_show_port_state(struct inv_mpu_state *st,char *buf){
...
static int diag_show_reference_register(struct inv_mpu_state *st,char *buf){
	return sprintf( buf, "%X %X\n",st->refreg.addr, st->refreg.reg );
}

static int diag_show_rw_register(struct inv_mpu_state *st,char *buf){
 ...
 * inv_diag_attr_store()
 */
static ssize_t inv_diag_attr_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct iio_dev *indio_dev = dev_get_drvdata(dev);
	struct inv_mpu_state *st = iio_priv(indio_dev);
	...
	switch (this_attr->address) {
	case ATTR_DIAG_RAW_GYRO:
		break;
	case ATTR_DIAG_RAW_ACCEL:
		break;
	case ATTR_DIAG_RAW_COMPASS:
		break;
	case ATTR_DIAG_POWER_STATE:
		result = diag_store_power_state(st, data);
		break;
	case ATTR_DIAG_GYRO_ENABLE:
		result = diag_store_gyro_enable(st, buf, count);
		break;
	case ATTR_DIAG_ACCL_ENABLE:
		result = diag_store_accel_enable(st, buf, count);
		break;
	case ATTR_DIAG_COMPASS_ENABLE:
		result = diag_store_compass_enable(st, buf, count);
		break;
	case ATTR_DIAG_REFERENCE_REGISTER:
		result = diag_store_reference_register(st, buf, count);
		break;
	case ATTR_DIAG_RW_REGISTET:
		result = diag_store_rw_register(st, buf, count);
		break;
	case ATTR_DIAG_COMPASS_SELF_TEST:
		break;
	}
 ...
 * inv_diag_attr_show()
 */
static ssize_t inv_diag_attr_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct iio_dev *indio_dev = dev_get_drvdata(dev);
	struct inv_mpu_state *st = iio_priv(indio_dev);
	...
	switch (this_attr->address) {
	case ATTR_DIAG_RAW_GYRO:
		result = diag_show_raw_gyro(st, buf);
		break;
	case ATTR_DIAG_RAW_ACCEL:
		result = diag_show_raw_accel(st, buf);
		break;
	case ATTR_DIAG_RAW_COMPASS:
		result = st->slave_compass->diag_read_data(st, buf);
		break;
	case ATTR_DIAG_POWER_STATE:
		result = diag_show_power_state(st, buf);
		break;
	case ATTR_DIAG_GYRO_ENABLE:
		result = diag_show_gyro_enable(st, buf);
		break;
	case ATTR_DIAG_ACCL_ENABLE:
		result = diag_show_accel_enable(st, buf);
		break;
	case ATTR_DIAG_COMPASS_ENABLE:
		result = diag_show_compass_enable(st, buf);
		break;
	case ATTR_DIAG_PORT_STATE:
		result = diag_show_port_state(st, buf);
		break;	
	case ATTR_DIAG_REFERENCE_REGISTER:
		result = diag_show_reference_register(st, buf);
		break;
	case ATTR_DIAG_RW_REGISTET:
		result = diag_show_rw_register(st, buf);
		break;
	case ATTR_DIAG_COMPASS_SELF_TEST:
		result = st->slave_compass->diag_self_test(st, buf);
		break;
	}
	
	return result;
}
/* SHMDS_ID_0201_01 add */
...
/* SHMDS_ID_0201_01 add S */
/* diag sysfs */
static IIO_DEVICE_ATTR(diag_raw_gyro, S_IRUGO | S_IWUSR, inv_diag_attr_show,
		inv_diag_attr_store, ATTR_DIAG_RAW_GYRO);
static IIO_DEVICE_ATTR(diag_raw_accl, S_IRUGO | S_IWUSR, inv_diag_attr_show,
		inv_diag_attr_store, ATTR_DIAG_RAW_ACCEL);
static IIO_DEVICE_ATTR(diag_raw_compass, S_IRUGO | S_IWUSR, inv_diag_attr_show,
		inv_diag_attr_store, ATTR_DIAG_RAW_COMPASS);
static IIO_DEVICE_ATTR(diag_power_state, S_IRUGO | S_IWUSR, inv_diag_attr_show,
		inv_diag_attr_store, ATTR_DIAG_POWER_STATE);
static IIO_DEVICE_ATTR(diag_gyro_enable, S_IRUGO | S_IWUSR, inv_diag_attr_show,
		inv_diag_attr_store, ATTR_DIAG_GYRO_ENABLE);
static IIO_DEVICE_ATTR(diag_accl_enable, S_IRUGO | S_IWUSR, inv_diag_attr_show,
		inv_diag_attr_store, ATTR_DIAG_ACCL_ENABLE);
static IIO_DEVICE_ATTR(diag_compass_enable, S_IRUGO | S_IWUSR, inv_diag_attr_show,
		inv_diag_attr_store, ATTR_DIAG_COMPASS_ENABLE);
static IIO_DEVICE_ATTR(diag_port_state, S_IRUGO | S_IWUSR, inv_diag_attr_show,
		inv_diag_attr_store, ATTR_DIAG_PORT_STATE);
static IIO_DEVICE_ATTR(diag_reference_register, S_IRUGO | S_IWUSR, inv_diag_attr_show,
		inv_diag_attr_store, ATTR_DIAG_REFERENCE_REGISTER);
static IIO_DEVICE_ATTR(diag_rw_registet, S_IRUGO | S_IWUSR, inv_diag_attr_show,
		inv_diag_attr_store, ATTR_DIAG_RW_REGISTET);
static IIO_DEVICE_ATTR(diag_compass_self_test, S_IRUGO | S_IWUSR, inv_diag_attr_show,
		inv_diag_attr_store, ATTR_DIAG_COMPASS_SELF_TEST);
/* SHMDS_ID_0201_01 add E */

/* SHMDS_ID_1101_01 add S */
/* for adb debug */
#if defined(SHMDS_DETECT) && defined(SHMDS_ADB_FLAG)
static IIO_DEVICE_ATTR(shmds_count, 0444, inv_attr_show, NULL, ATTR_SHMDS_COUNT);
static IIO_DEVICE_ATTR(shmds_ths_value, 0644, inv_attr_show, inv_dmp_attr_store, ATTR_SHMDS_THS_VALUE);
static IIO_DEVICE_ATTR(shmds_duration_value, 0644, inv_attr_show, inv_dmp_attr_store, ATTR_SHMDS_DURATION_VALUE);
static IIO_DEVICE_ATTR(shmds_buf_size_temp, 0644, inv_attr_show, inv_dmp_attr_store, ATTR_SHMDS_BUF_SIZE_TEMP);
static IIO_DEVICE_ATTR(shmds_md_thresh, 0644, inv_attr_show, inv_dmp_attr_store, ATTR_SHMDS_MD_THRESH);
static IIO_DEVICE_ATTR(shmds_mg_to_LSB, 0644, inv_attr_show, inv_dmp_attr_store, ATTR_SHMDS_MG_TO_LSB);
static IIO_DEVICE_ATTR(shmds_md_thresh_value, 0644, inv_attr_show, inv_dmp_attr_store, ATTR_SHMDS_MD_THRESH_VALUE);
#endif /* SHMDS_DETECT, SHMDS_ADB_FLAG */
/* SHMDS_ID_1101_01 add E */
...
/* for adb debug */
#if defined(SHMDS_DETECT) && defined(SHMDS_ADB_FLAG)
    &iio_dev_attr_shmds_count.dev_attr.attr,
    &iio_dev_attr_shmds_ths_value.dev_attr.attr,
    &iio_dev_attr_shmds_duration_value.dev_attr.attr,
    &iio_dev_attr_shmds_buf_size_temp.dev_attr.attr,
    &iio_dev_attr_shmds_md_thresh.dev_attr.attr,
    &iio_dev_attr_shmds_mg_to_LSB.dev_attr.attr,
    &iio_dev_attr_shmds_md_thresh_value.dev_attr.attr,
#endif /* SHMDS_DETECT, SHMDS_ADB_FLAG */
/* SHMDS_ID_1101_01 add E */
};
...
/* SHMDS_ID_0201_01 add S */
static const struct attribute *inv_diag_attributes[] = {
	&iio_dev_attr_diag_raw_gyro.dev_attr.attr,
	&iio_dev_attr_diag_raw_accl.dev_attr.attr,
	&iio_dev_attr_diag_raw_compass.dev_attr.attr,
	&iio_dev_attr_diag_power_state.dev_attr.attr,
	&iio_dev_attr_diag_gyro_enable.dev_attr.attr,
	&iio_dev_attr_diag_accl_enable.dev_attr.attr,
	&iio_dev_attr_diag_compass_enable.dev_attr.attr,
	&iio_dev_attr_diag_port_state.dev_attr.attr,
	&iio_dev_attr_diag_reference_register.dev_attr.attr,
	&iio_dev_attr_diag_rw_registet.dev_attr.attr,
	&iio_dev_attr_diag_compass_self_test.dev_attr.attr,
};
/* SHMDS_ID_0201_01 add E */
...
/* SHMDS_ID_0201_01 add S */
	ARRAY_SIZE(inv_diag_attributes) +
/* SHMDS_ID_0201_01 add E */
	1
];
...
/* SHMDS_ID_0201_01 add S */	
	memcpy(&inv_attributes[t_ind], inv_diag_attributes,
		sizeof(inv_diag_attributes));
	t_ind += ARRAY_SIZE(inv_diag_attributes);
/* SHMDS_ID_0201_01 add E */
...
#endif /* CONFIG_DTS_INV_MPU_IIO */
/* SHMDS_ID_0101_09 add E */
...
/* SHMDS_ID_0207_01 add S */
	mutex_init(&smd_timerlock);
	init_timer(&restart_smd_timer);
	restart_smd_timer.function = restart_ped_timer_handler;
/* SHMDS_ID_0207_01 add E */

/* SHMDS_ID_1903_01 mod S */
//#ifdef CONFIG_DTS_INV_MPU_IIO
//	enable_irq_wake(client->irq);
//#endif
	enable_irq( client->irq );
/* SHMDS_ID_1903_01 mod E */
...
/* SHMDS_ID_1101_01 add S */
#ifdef SHMDS_DETECT
    result = shmds_mot_init(st, client->irq);
    if (result)
    {
        pr_err("shmds_mot_init fail\n");
        goto out_unreg_iio;
    }
#endif /* SHMDS_DETECT */
/* SHMDS_ID_1101_01 add E */
...
/* SHMDS_ID_0112_01 del E */
	...
	if (st->chip_config.dmp_on && st->chip_config.enable) {
		...
		result = st->set_power_state(st, true);
/* SHMDS_ID_1705_01 add S */
		usleep(STANDBY_WAIT_UTIME - REG_UP_UTIME);
		inv_reset_fifo(indio_dev);
/* SHMDS_ID_1705_01 add E */
	}
/* SHMDS_ID_1903_01 add S */
	disable_irq_wake( st->irq );
	enable_irq( st->irq );
/* SHMDS_ID_1903_01 add E */
...
/* SHMDS_ID_1101_01 add S */
#ifdef SHMDS_DETECT
	shmds_check_mot_move(indio_dev, s, 1);
#endif /* SHMDS_DETECT */
/* SHMDS_ID_1101_01 add E */
	result = 0;
	if (st->chip_config.dmp_on && st->chip_config.enable) {
...
/* SHMDS_ID_0112_01 add S */
	mutex_unlock(&indio_dev->mlock);
/* SHMDS_ID_0112_01 add E */
/* SHMDS_ID_0112_01 mod S */
//	st->suspend_state = true;
	st->suspend_state = false;
/* SHMDS_ID_0112_01 mod E */
	/* add code according to different request End */
#ifndef USE_SUSPEND_NOIRQ
/* SHMDS_ID_0112_01 del S */

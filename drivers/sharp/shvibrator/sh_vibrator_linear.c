...
#define SH_VIB_RESET_PORT	8

#define SH_VIB_START_WAIT_TIME	(300)
#define SH_VIB_STOP_WAIT_TIME	(30 * 1000)

#define SH_VIB_DEFAULT_TIMEOUT	15000
#define SH_VIB_PM_QOS_LATENCY_VALUE	1
...
static struct pm_qos_request shvibrator_qos_cpu_dma_latency;
...
static void shvibrator_set(int on)
{
	...
	if (on) {
		...
		/* Reset Port "High" */
		gpio_set_value( SH_VIB_RESET_PORT, 1 );
		...
		if (vib_data) {
			...
			if( (boot_mode == SH_BOOT_D) || (boot_mode == SH_BOOT_F_F) ) {
				...
				if (smemzero == false) {
					for (i = 0; i < 9; i++) {
						if (shvibrator_i2c_write(vib_data->client_p, i+1, param[i]) < 0) goto set_pmic_vibrator_i2c_err;
					}
				} else {
#ifdef CONFIG_MACH_REN
					if (shvibrator_i2c_write(vib_data->client_p, 0x01, 0x08) < 0) goto set_pmic_vibrator_i2c_err;
					if (shvibrator_i2c_write(vib_data->client_p, 0x02, 0x10) < 0) goto set_pmic_vibrator_i2c_err;
					if (shvibrator_i2c_write(vib_data->client_p, 0x03, 0x07) < 0) goto set_pmic_vibrator_i2c_err;
					if (shvibrator_i2c_write(vib_data->client_p, 0x04, 0x00) < 0) goto set_pmic_vibrator_i2c_err;
					if (shvibrator_i2c_write(vib_data->client_p, 0x05, 0x0A) < 0) goto set_pmic_vibrator_i2c_err;
					if (shvibrator_i2c_write(vib_data->client_p, 0x08, 0x04) < 0) goto set_pmic_vibrator_i2c_err;
#else
					if (shvibrator_i2c_write(vib_data->client_p, 0x01, 0x0A) < 0) goto set_pmic_vibrator_i2c_err;
					if (shvibrator_i2c_write(vib_data->client_p, 0x02, 0x10) < 0) goto set_pmic_vibrator_i2c_err;
					if (shvibrator_i2c_write(vib_data->client_p, 0x03, 0x05) < 0) goto set_pmic_vibrator_i2c_err;
					if (shvibrator_i2c_write(vib_data->client_p, 0x04, 0x2F) < 0) goto set_pmic_vibrator_i2c_err;
#endif
				}
			} else {
#ifdef CONFIG_MACH_REN
		...
		/* Reset Port "Low" */
		gpio_set_value( SH_VIB_RESET_PORT, 0 );
		pm_qos_update_request(&shvibrator_qos_cpu_dma_latency, PM_QOS_DEFAULT_VALUE);
		SHVIBRATOR_DBG_LOG("shvibrator_qos_cpu_dma_latency : unlock ");
...
static void shvibrator_enable(struct timed_output_dev *dev, int value)
{
	...
	if (value == 0)
		...
		pm_qos_update_request(&shvibrator_qos_cpu_dma_latency, SH_VIB_PM_QOS_LATENCY_VALUE);
		SHVIBRATOR_DBG_LOG("shvibrator_qos_cpu_dma_latency : lock ");
...
#ifdef CONFIG_MACH_REN
		if( shvibrator_i2c_write(vib_data->client_p, 0x03, 0x07) < 0)
			i2c_err++;
		if( shvibrator_i2c_read(vib_data->client_p, 0x03, &getval ,1) < 0)
			i2c_err++;
		if ((getval == 0x07) && (i2c_err == 0))
			rc = 0;
#else
...
static int __init shvibrator_init(void)
{
	...
	pm_qos_add_request(&shvibrator_qos_cpu_dma_latency, PM_QOS_CPU_DMA_LATENCY, PM_QOS_DEFAULT_VALUE);
...
static void __exit shvibrator_exit(void)
{
	...
	pm_qos_remove_request(&shvibrator_qos_cpu_dma_latency);
}

...
#define	SHIRDA_PERFLOCK_FREQ	SHIRDA_PERFLOCK_MINFREQ
					...
					unsigned long event, void *data)
{
	...
	switch (event) {
	case CPUFREQ_ADJUST:
		cpufreq_verify_within_limits(policy, shirda_perflock_freq,
								UINT_MAX);
		break;
	default:
		break;
...
static int shirda2_open(struct inode *inode, struct file *fp)
{
	...
	if (ret != 0) {
		...
		shirda_perflock_freq = SHIRDA_PERFLOCK_FREQ;
		for_each_online_cpu(cpu) {
			cpufreq_update_policy(cpu);
		}

		IRDALOG_INFO("CPUFREQ perflock.\n");
...
static int shirda2_release(struct inode *inode, struct file *fp)
{
	...
	IRDALOG_INFO("CPUFREQ perfunlock.\n");

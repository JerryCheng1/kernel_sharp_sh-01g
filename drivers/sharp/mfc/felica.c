 ...
 *
 * Copyright (C) 2010-2014 SHARP CORPORATION
 *
...
#define D_RFS_WAKE_LOCK_TIMEOUT	((HZ*4)/5)	/* 800msec */
...
#define D_INT_WAKE_LOCK_TIMEOUT	((HZ*4)/5)	/* 800msec */
...
static struct wake_lock g_rfs_wake_lock;
...
/* COORDINATOR Qualcomm1021 BUILDERR MODIFY end */
	...
	if (wake_lock_active(&g_rfs_wake_lock)) {
		wake_unlock(&g_rfs_wake_lock);
	}
	wake_lock_timeout(&g_rfs_wake_lock, D_RFS_WAKE_LOCK_TIMEOUT);
...
static int rfs_poll_init(void)
{
    ...
    wake_lock_init(&g_rfs_wake_lock, WAKE_LOCK_SUSPEND, "rfs_wake_lock");
...
static void rfs_poll_exit(void)
{
    ...
    wake_lock_destroy(&g_rfs_wake_lock);

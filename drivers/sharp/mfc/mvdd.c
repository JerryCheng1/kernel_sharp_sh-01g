...
#define D_SNFCEN_POWCTRL_TRUE	(1)
#define D_SNFCEN_POWCTRL_FALSE	(0)
...
static int snfc_en_powctrl_flg = D_SNFCEN_POWCTRL_FALSE;
...
static void snfc_output_enable(void)
{
	...
	snfc_en_powctrl_flg = D_SNFCEN_POWCTRL_TRUE;
...
static void snfc_output_disable(void)
{
	...
	snfc_en_powctrl_flg = D_SNFCEN_POWCTRL_FALSE;
		       ...
		       size_t len, loff_t *ppos)
{
	...
	if(on == 0){
		/* disable */
		if(snfc_en_powctrl_flg == D_SNFCEN_POWCTRL_TRUE){
			snfc_output_disable();
		}
	}else if(on == 1){
		/* enable */
		if((snfc_en_powctrl_flg == D_SNFCEN_POWCTRL_FALSE)&&(snfc_available() == D_MVDD_ENABLE)){
			snfc_output_enable();
		}
	}

...
{
    memcpy(&(shdisp_pm_ctx.photo_sensor_adj),
...
/* ------------------------------------------------------------------------- */
/* shdisp_pm_init_recovery_subscribe                                         */
/* ------------------------------------------------------------------------- */
void shdisp_pm_init_recovery_subscribe(void)
{
    SHDISP_DEBUG("shdisp_pm_ctx.psals_status.power_status=%d", shdisp_pm_ctx.psals_status.power_status);
    if ((shdisp_pm_ctx.psals_status.power_status == SHDISP_DEV_STATE_INIT) ||
...
int shdisp_pm_clmr_power_manager(int type, int state)
{
    ...
    SHDISP_TRACE("in type:0x%08X, state:%s, clmr_status:%s, users:0x%08X\n",
            ...
            (shdisp_pm_ctx.clmr_status.power_status ?
            ((shdisp_pm_ctx.clmr_status.power_status == SHDISP_DEV_STATE_OFF) ? "off" : "on") : "noinit"),
...
int shdisp_pm_psals_power_manager(int type, int state)
{
    ...
    if (shdisp_pm_ctx.psals_status.power_status != state) {
        if ((state == SHDISP_DEV_STATE_ON) &&
            (shdisp_pm_ctx.psals_status.power_status == SHDISP_DEV_STATE_INIT)) {
            shdisp_pm_ctx.psals_status.power_status = SHDISP_DEV_STATE_ON;
        } else if ((state == SHDISP_DEV_STATE_INIT) &&
            (shdisp_pm_ctx.psals_status.power_status == SHDISP_DEV_STATE_ON)) {
            ;
        } else if ((state == SHDISP_DEV_STATE_ON) ||
                   (state == SHDISP_DEV_STATE_INIT)) {
...
static int shdisp_pm_psals_ps_init(void)
{
    ...
    switch (state) {
    case SHDISP_SENSOR_STATE_PROX_OFF_ALS_OFF:
        ret = shdisp_bdic_API_psals_ps_init_als_off();
        if (ret == SHDISP_RESULT_SUCCESS) {
            shdisp_pm_psals_set_state(SHDISP_SENSOR_STATE_PROX_ON_ALS_OFF);
        }
        break;
    ...
    case SHDISP_SENSOR_STATE_PROX_OFF_ALS_ON:
        ...
        if (ret == SHDISP_RESULT_SUCCESS) {
            shdisp_pm_psals_set_state(SHDISP_SENSOR_STATE_PROX_ON_ALS_ON);
        }
        break;
...
static int shdisp_pm_psals_ps_deinit(void)
{
    ...
    case SHDISP_SENSOR_STATE_PROX_ON_ALS_OFF:
        ...
        if (ret == SHDISP_RESULT_SUCCESS) {
            shdisp_pm_psals_set_state(SHDISP_SENSOR_STATE_PROX_OFF_ALS_OFF);
        }
        break;
    case SHDISP_SENSOR_STATE_PROX_OFF_ALS_ON:
        break;
    case SHDISP_SENSOR_STATE_PROX_ON_ALS_ON:
        ...
        if (ret == SHDISP_RESULT_SUCCESS) {
            shdisp_pm_psals_set_state(SHDISP_SENSOR_STATE_PROX_OFF_ALS_ON);
        }
        break;
    default:
        SHDISP_ERR("<RESULT_FAILURE> invalid state %d.\n", state);

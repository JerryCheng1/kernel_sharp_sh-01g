...
/* -------------------------------------------------------------------------- */
static u8 shtps_filter_edge_fail_touch_top_area_check(unsigned short x, unsigned short y)
{
	u8 is_top_area = 0;
	...
	for(i = 0; i < fingerMax; i++){
		...
		if( (ts->edge_fail_touch_rej_p->edge_fail_touch_top_inhibit_id & (1 << i)) != 0 ){
			if(info->fingers[i].state == SHTPS_TOUCH_STATE_FINGER){
				if( (shtps_filter_edge_fail_touch_top_area_check(ts->fw_report_info.fingers[i].x, ts->fw_report_info.fingers[i].y) == 0 ) ){
					ts->edge_fail_touch_rej_p->edge_fail_touch_top_inhibit_id &= ~(1 << i);

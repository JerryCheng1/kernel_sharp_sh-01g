...
static void shtps_report_touch_on(struct input_dev *input_dev, int id, int tool, int x, int y, int w, int z, int orientation);
static void shtps_report_touch_off(struct input_dev *input_dev, int id, int tool, int x, int y, int w, int z, int orientation);
...
static void shtps_set_touch_info(struct shtps_mxt *ts)
{
	...
	for(i = 0;i < fingerMax;i++){
		if(report_fingers[i].state == TPSDEV_INPUT_STATE_ON){
			SHTPS_LOG_EVENT(
				DBG_PRINTK("[%s]Touch Info[%d] x=%d, y=%d, w=%d, z=%d\n",
						report_fingers[i].tool == MT_TOOL_FINGER ? "Finger" :
						report_fingers[i].tool == MT_TOOL_PEN ? "Pen" : "Other" ,
						i,
						report_fingers[i].x,
						report_fingers[i].y,
						report_fingers[i].w,
		...
		}else if(report_pre_fingers[i].state == TPSDEV_INPUT_STATE_ON){
			SHTPS_LOG_EVENT(
				DBG_PRINTK("[%s]Touch Info[%d] x=%d, y=%d, w=%d, z=%d\n",
						"NoTouch",
						i,
...
//	shtps_filter_main(ts);
	...
	for (i = 0; i < fingerMax; i++) {
		if(report_pre_fingers[i].state == TPSDEV_INPUT_STATE_ON){
			numOfPreFingers++;
		}
	}
...
static void shtps_report_touch_on(struct input_dev *input_dev, int id, int tool, int x, int y, int w, int z, int orientation)
{
	...
	SHTPS_LOG_EVENT(
		DBG_PRINTK("[%s]Notify event[%d] touch=100(%d), x=%d(%d), y=%d(%d) w=%d\n",
						tool == MT_TOOL_FINGER ? "finger" :
						tool == MT_TOOL_PEN ? "pen" : "other" ,
						id, z, x, x, y, y, w);
	);
}
...
static void shtps_report_touch_off(struct input_dev *input_dev, int id, int tool, int x, int y, int w, int z, int orientation)
{
	...
	SHTPS_LOG_EVENT(
		DBG_PRINTK("[%s]Notify event[%d] touch=0(%d), x=%d(%d), y=%d(%d) w=%d\n",
						tool == MT_TOOL_FINGER ? "finger" :
						tool == MT_TOOL_PEN ? "pen" : "other" ,
						id, z, x, x, y, y, w);
	);
	...
	int w_enable, int w, int z_enable, int z, int ori_enable, int orientation)
{
	...
	fw_cur_fingers[id].state = (state)? TPSDEV_INPUT_STATE_ON : TPSDEV_INPUT_STATE_OFF;
	fw_cur_fingers[id].tool = (tool == MT_TOOL_FINGER)? TPSDEV_INPUT_ITEM_TOOL_FINGER : TPSDEV_INPUT_ITEM_TOOL_PEN;
	...
	fw_cur_fingers[id].orientation = (ori_enable)? orientation : TPSDEV_INPUT_ITEM_INVALID;
		...
		if(shtps_touch_emu_is_recording() != 0){
			shtps_touch_emu_rec_finger_info(id, &fw_cur_fingers[id]);
		}
...
void shtps_mxt_input_force_touchup(struct input_dev *input_dev)
{
	...
	for(i = 0;i < SHTPS_FINGER_MAX;i++){
		...
		report_fingers[i].state = TPSDEV_INPUT_STATE_OFF;
		fw_fingers[i].state = TPSDEV_INPUT_STATE_OFF;
	}

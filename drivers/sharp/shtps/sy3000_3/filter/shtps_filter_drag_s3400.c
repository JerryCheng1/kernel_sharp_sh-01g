			...
			if(ts->report_info.fingers[i].state != SHTPS_TOUCH_STATE_NO_TOUCH){
				if(ts->drag_hist_p->drag_hist[i][xy].count >= SHTPS_DRAG_SMOOTH_COUNT_MIN){
					...
					if(SHTPS_DRAG_SMOOTH_USE_HULL){
						// HULL 1st
						for(j = ts->drag_hist_p->drag_hist[i][xy].count / 2; j < ts->drag_hist_p->drag_hist[i][xy].count - 1; j++){
							temp_HULL_numerator += ts->drag_hist_p->drag_hist[i][xy].history_gap[j] * (j-4);
						...
						if(SHTPS_DRAG_SMOOTH_HULL_MULTI){
							...
							// HULL 2nd
							for(j = ts->drag_hist_p->drag_hist[i][xy].count / 2; j < ts->drag_hist_p->drag_hist[i][xy].count - 1; j++){
								temp_HULL_numerator += ts->drag_hist_p->drag_hist[i][xy].hull_1st_gap[j] * (j-4);
								temp_HULL_denominator += j - 4;
							}
							inc_ave = 2 * temp_HULL_numerator / temp_HULL_denominator;
					...
					if(xy == SHTPS_POSTYPE_X){
						if(ts->drag_hist_p->drag_hist[i][xy].history_old == last_history){
							...
							if(!ts->drag_hist_p->drag_hist[i][xy].skip_comp){
		...
		if(SHTPS_DRAG_SMOOTH_USE_HULL){
			...
			for(i = 0;i < fingerMax;i++){
				...
				if((diagonal_gap_fixed[i] > 0-exclude_dot_fixed) && (diagonal_gap_fixed[i] < exclude_dot_fixed)){
					...
					ts->drag_hist_p->drag_hist[i][SHTPS_POSTYPE_X].skip_comp = false;
					ts->drag_hist_p->drag_hist[i][SHTPS_POSTYPE_Y].skip_comp = false;
				}
			}
		}

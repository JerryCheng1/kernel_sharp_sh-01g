...
{
	SHTPS_LOG_DBG_PRINT("shtps_cpu_clock_ctrl_lock");
...
static inline void shtps_init_drag_hist(struct shtps_rmi_spi *ts, int xy, int finger, int pos)
{
	...
	ts->drag_hist[finger][xy].count = 0;
		...
		ts->drag_hist[finger][xy].history_old = 0;
	#endif /* SHTPS_DRAG_SMOOTH_ENABLE */
}
	...
	if(dir != SHTPS_DRAG_DIR_NONE){
		if(ts->drag_hist[finger][xy].count == 0){
				...
				if(SHTPS_DRAG_SMOOTH){
					...
					ts->drag_hist[finger][xy].history_old = pos;
				}
			#endif /* SHTPS_DRAG_SMOOTH_ENABLE */
			...
			ts->drag_hist[finger][xy].count = 1;
				...
				if(SHTPS_DRAG_SMOOTH){
					...
					drag_smooth_count_limit_new = 
						...
						(ts->drag_hist[finger][xy].count_up_base / SHTPS_DRAG_SMOOTH_COUNT_UP_STEP);
					...
					if(ts->drag_hist[finger][xy].dir != dir){
						...
						if(drag_smooth_count_limit < ts->drag_hist[finger][xy].count){
							for(i= 0; i < drag_smooth_count_limit; i++){
								ts->drag_hist[finger][xy].history[i] = 
									ts->drag_hist[finger][xy].history[ts->drag_hist[finger][xy].count - 
					...
					if(ts->drag_hist[finger][xy].count < SHTPS_DRAG_SMOOTH_COUNT_MIN-1){
						if(SHTPS_DRAG_SMOOTH_USE_HULL){
							ts->drag_hist[finger][xy].history_gap[ts->drag_hist[finger][xy].count-1] = pos - ts->drag_hist[finger][xy].history[ts->drag_hist[finger][xy].count-1];
							if(SHTPS_DRAG_SMOOTH_HULL_MULTI){
								ts->drag_hist[finger][xy].hull_1st_gap[ts->drag_hist[finger][xy].count-1] = pos - ts->drag_hist[finger][xy].history[ts->drag_hist[finger][xy].count-1];
							}
						}
						ts->drag_hist[finger][xy].history[ts->drag_hist[finger][xy].count] = pos;
						...
						if(ts->drag_hist[finger][xy].count == drag_smooth_count_limit-1){
							...
							for(i= 0; i < drag_smooth_count_limit-1; i++){
								ts->drag_hist[finger][xy].history[i] = 
									ts->drag_hist[finger][xy].history[i+1];
							}

							if(SHTPS_DRAG_SMOOTH_USE_HULL){
								for(i= 0; i < drag_smooth_count_limit-2; i++){
									ts->drag_hist[finger][xy].history_gap[i] = 
										ts->drag_hist[finger][xy].history_gap[i+1];
								}
								ts->drag_hist[finger][xy].history_gap[drag_smooth_count_limit-2] = pos - ts->drag_hist[finger][xy].history[ts->drag_hist[finger][xy].count-2];
							}
				...
				(ts->report_info.fingers[i].state == SHTPS_TOUCH_STATE_NO_TOUCH) )
			{
				...
				if( (shtps_edge_fail_touch_top_area_check(ts->fw_report_info.fingers[i].x, ts->fw_report_info.fingers[i].y) != 0) ){
					if(ts->fw_report_info.fingers[i].z <= SHTPS_EDGE_FAIL_TOUCH_REJECT_TOP_AREA_THRESH_Z)
					{
						...
						if( ((ts->fw_report_info.fingers[i].z * 10) / w) <= SHTPS_EDGE_FAIL_TOUCH_REJECT_TOP_AREA_THRESH_ZW_RATIO ){
							ts->edge_fail_touch_top_inhibit_id |= (1 << i);
			...
			if( (ts->grip_fail_touch_inhibit_id & (1 << i)) == 0 ){
					...
					(ts->report_info.fingers[i].state == SHTPS_TOUCH_STATE_NO_TOUCH) )
				{
					if( (ts->fw_report_info.fingers[i].x < SHTPS_GRIP_FAIL_TOUCH_REJECT_AREA) ||
						(ts->fw_report_info.fingers[i].x > (CONFIG_SHTPS_SY3000_LCD_SIZE_X - 1 - SHTPS_GRIP_FAIL_TOUCH_REJECT_AREA)) )
					{
						if(ts->fw_report_info.fingers[i].wy >= SHTPS_GRIP_FAIL_TOUCH_REJECT_W_THRESH_MAX){
							ts->grip_fail_touch_inhibit_id |= (1 << i);
						}else if(ts->fw_report_info.fingers[i].wy >= SHTPS_GRIP_FAIL_TOUCH_REJECT_W_THRESH_MIN){
...
static int shtps_is_low_frame_rate_mode(struct shtps_rmi_spi *ts)
{
	return 0;
}
		...
		if(info->fingers[i].state == SHTPS_TOUCH_STATE_FINGER /* info->fingers[i].state == SHTPS_TOUCH_STATE_PEN */){
			if(ts->report_info.fingers[i].state != SHTPS_TOUCH_STATE_NO_TOUCH){
				if(ts->drag_hist[i][xy].count >= SHTPS_DRAG_SMOOTH_COUNT_MIN){
					...
					if(SHTPS_DRAG_SMOOTH_USE_HULL){
						// HULL 1st
						for(j = ts->drag_hist[i][xy].count / 2; j < ts->drag_hist[i][xy].count - 1; j++){
							temp_HULL_numerator += ts->drag_hist[i][xy].history_gap[j] * (j-4);
							temp_HULL_denominator += j - 4;
						}
						...
						for(j = 0; j < ts->drag_hist[i][xy].count - 1; j++){
							temp_HULL_numerator += ts->drag_hist[i][xy].history_gap[j] * (j+1);
						...
						if(SHTPS_DRAG_SMOOTH_HULL_MULTI){
							...
							for(j= 0; j < ts->drag_hist[i][xy].count-2; j++){
								ts->drag_hist[i][xy].hull_1st_gap[j] = ts->drag_hist[i][xy].hull_1st_gap[j+1];
							}
							ts->drag_hist[i][xy].hull_1st_gap[ts->drag_hist[i][xy].count-2] = inc_ave;

							// HULL 2nd
							for(j = ts->drag_hist[i][xy].count / 2; j < ts->drag_hist[i][xy].count - 1; j++){
								temp_HULL_numerator += ts->drag_hist[i][xy].hull_1st_gap[j] * (j-4);
								temp_HULL_denominator += j - 4;
							}
							inc_ave = 2 * temp_HULL_numerator / temp_HULL_denominator;
							...
							for(j = 0; j < ts->drag_hist[i][xy].count - 1; j++){
								temp_HULL_numerator += ts->drag_hist[i][xy].hull_1st_gap[j] *  (j+1);
								temp_HULL_denominator += j + 1;
							}
						...
						if(xy == SHTPS_POSTYPE_X){
							if(SHTPS_DRAG_SMOOTH_HULL_MULTI){
								SHTPS_LOG_DRAG_SMOOTH("[DEBUG_X]HULL_1st=%d HULL_2nd=%d\n", 
										ts->drag_hist[i][xy].hull_1st_gap[ts->drag_hist[i][xy].count-2],
										inc_ave);
							}else{
							...
							SHTPS_LOG_DRAG_SMOOTH("[DEBUG_Y]real_gap %d %d %d %d %d %d %d %d %d %d\n", 
									ts->drag_hist[i][xy].history_gap[0],
									ts->drag_hist[i][xy].history_gap[1],
									ts->drag_hist[i][xy].history_gap[2],
									ts->drag_hist[i][xy].history_gap[3],
									ts->drag_hist[i][xy].history_gap[4],
									ts->drag_hist[i][xy].history_gap[5],
									ts->drag_hist[i][xy].history_gap[6],
									ts->drag_hist[i][xy].history_gap[7],
									ts->drag_hist[i][xy].history_gap[8],
									ts->drag_hist[i][xy].history_gap[9]);

							if(SHTPS_DRAG_SMOOTH_HULL_MULTI){
								SHTPS_LOG_DRAG_SMOOTH("[DEBUG_Y]HULL_1st %d %d %d %d %d %d %d %d %d %d\n", 
										ts->drag_hist[i][xy].hull_1st_gap[0],
										ts->drag_hist[i][xy].hull_1st_gap[1],
										ts->drag_hist[i][xy].hull_1st_gap[2],
										ts->drag_hist[i][xy].hull_1st_gap[3],
										ts->drag_hist[i][xy].hull_1st_gap[4],
										ts->drag_hist[i][xy].hull_1st_gap[5],
										ts->drag_hist[i][xy].hull_1st_gap[6],
										ts->drag_hist[i][xy].hull_1st_gap[7],
										ts->drag_hist[i][xy].hull_1st_gap[8],
										ts->drag_hist[i][xy].hull_1st_gap[9]);

								SHTPS_LOG_DRAG_SMOOTH("[DEBUG_Y]real_gap=%d HULL_1st=%d HULL_2nd=%d\n", 
										ts->drag_hist[i][xy].history_gap[ts->drag_hist[i][xy].count-2],
										ts->drag_hist[i][xy].hull_1st_gap[ts->drag_hist[i][xy].count-2],
										inc_ave);
							}else{
						...
						inc_ave_FIXED = ((ts->drag_hist[i][xy].dir == SHTPS_DRAG_DIR_PLUS && inc_ave_FIXED < 0) ||
										 (ts->drag_hist[i][xy].dir == SHTPS_DRAG_DIR_MINUS && inc_ave_FIXED > 0)) ? 0 : inc_ave_FIXED;
					}
					else{
						inc_ave_FIXED = 
							SHTPS_DRAG_SMOOTH_INT_TO_FIXED(
								last_history- ts->drag_hist[i][xy].history[0]) / 
								(ts->drag_hist[i][xy].count-1);
					}
					...
					if(xy == SHTPS_POSTYPE_X){
						if(ts->drag_hist[i][xy].history_old == last_history){
							...
							if(!ts->drag_hist[i][xy].skip_comp){
								...
								temp_FIXED = ts->drag_hist[i][xy].pre_comp_history_FIXED + ts->drag_hist[i][xy].real_gap_FIXED;
							}
...
static void shtps_flattery_PutHistory(struct shtps_rmi_spi *ts, struct RawPointerData_Pointer *inPointer){
	...
	for(xy = 0 ; xy < 2 ; xy++){
		pOriginalHistory = &ts->flatteryOriginalHistory[xy][inPointer->id];
		if(xy == SHTPS_POSTYPE_X){
		...
		if(pOriginalHistory->cnt_available_data < SHTPS_DRAG_SMOOTH_FLATTERY_SMOOTH_SAMPLE_COUNT){
			if(pOriginalHistory->cnt_available_data == 0){
				pOriginalHistory->denominator = SHTPS_DRAG_SMOOTH_FLATTERY_WEIGHTED_MOVING_AVE_DENOMINATOR + 1;
			}
...
static void shtps_flattery_DragSmoothRevise(struct shtps_rmi_spi *ts, struct RawPointerData_Pointer *inPointer){
	...
	for(xy = 0 ; xy < 2 ; xy++){
		...
		if(pOriginalHistory->cnt_available_data >= SHTPS_DRAG_SMOOTH_FLATTERY_SMOOTH_SAMPLE_COUNT){
			if(SHTPS_DRAG_SMOOTH_FLATTERY_RATIO < pOriginalHistory->denominator){
				pOriginalHistory->denominator--;
			}else if(SHTPS_DRAG_SMOOTH_FLATTERY_RATIO >= pOriginalHistory->denominator){
				pOriginalHistory->denominator = SHTPS_DRAG_SMOOTH_FLATTERY_RATIO;
			}
...
static void shtps_flattery_DragLimitRevise(struct shtps_rmi_spi *ts, struct RawPointerData_Pointer *inPointer){
	...
	for(xy = 0 ; xy < 2 ; xy++){
		...
		if(xy == SHTPS_POSTYPE_X){
			input_coordinate = &(inPointer->x);
			disp_size = CONFIG_SHTPS_SY3000_PANEL_SIZE_X;
		}else{
			input_coordinate = &(inPointer->y);
			disp_size = CONFIG_SHTPS_SY3000_PANEL_SIZE_Y;
		}
...
static void shtps_flattery_ClearHistory(struct shtps_rmi_spi *ts, int clear_all_flg, int *InUse_flg){
	...
	for(xy = 0; xy < 2; xy++){
		for(i = 0; i < SHTPS_FINGER_MAX; i++){
			...
			ts->flatteryOriginalHistory[xy][i].cnt_available_data = 0;
		}
	}
}
	...
	for(i = 0;i < ts->coaxial_ghost_reject.pending_cnt;i++){
		for(j = 1;j < shtps_get_fingermax(ts);j++){
			if(ts->coaxial_ghost_reject.pending_ghost[i][j] != 0xFF){
				u8 finger = ts->coaxial_ghost_reject.pending_ghost[i][0];
				...
				SHTPS_LOG_DBG_PRINT("[COAXIAL_GHOST] reject ghost. [%d] x=%d, y=%d, wx=%d, wy=%d, z=%d "
											...
											finger, 
											ts->fw_report_info.fingers[finger].x, 
											ts->fw_report_info.fingers[finger].y,
											ts->fw_report_info.fingers[finger].wx, 
											ts->fw_report_info.fingers[finger].wy,
											ts->fw_report_info.fingers[finger].z);
			}else{
				break;
			}
		}
...
static void shtps_calc_notify(struct shtps_rmi_spi *ts, u8 *buf, struct shtps_touch_info *info, u8 *event)
{
	...
	if(SHTPS_DRAG_SMOOTH){
		...
		int	diagonal_gap_fixed[SHTPS_FINGER_MAX];
		int	exclude_dot;
		int	exclude_dot_fixed;

		if(SHTPS_DRAG_SMOOTH_USE_HULL){
			if(shtps_is_low_frame_rate_mode(ts) == 0){
				exclude_dot = SHTPS_DRAG_SMOOTH_EXCLUDE_DOT;
			}
			else{
				exclude_dot = SHTPS_DRAG_SMOOTH_EXCLUDE_DOT_LOW_FPS;
			}
			exclude_dot_fixed = SHTPS_DRAG_SMOOTH_INT_TO_FIXED(exclude_dot);
			...
			for(i = 0;i < fingerMax;i++){
				if(ts->drag_hist[i][SHTPS_POSTYPE_X].count > 0){
					x_gap_fixed[i] = SHTPS_DRAG_SMOOTH_INT_TO_FIXED(info->fingers[i].x) - SHTPS_DRAG_SMOOTH_INT_TO_FIXED(ts->drag_hist[i][SHTPS_POSTYPE_X].history_old);
				}
				else{
					x_gap_fixed[i] = 0;
				}
				if(ts->drag_hist[i][SHTPS_POSTYPE_Y].count > 0){
					y_gap_fixed[i] = SHTPS_DRAG_SMOOTH_INT_TO_FIXED(info->fingers[i].y) - SHTPS_DRAG_SMOOTH_INT_TO_FIXED(ts->drag_hist[i][SHTPS_POSTYPE_Y].history_old);
				}
				else{
					y_gap_fixed[i] = 0;
				}
				diagonal_gap_fixed[i] = shtps_sqrt((x_gap_fixed[i] * x_gap_fixed[i]) + (y_gap_fixed[i] * y_gap_fixed[i]));

				SHTPS_LOG_DRAG_SMOOTH("[DEBUG_SMOOTH]diagonal_gap[%d] = %d\n", i, SHTPS_DRAG_SMOOTH_FIXED_TO_INT(diagonal_gap_fixed[i]));

				if((diagonal_gap_fixed[i] > 0-exclude_dot_fixed) && (diagonal_gap_fixed[i] < exclude_dot_fixed)){
					SHTPS_LOG_DRAG_SMOOTH("[DEBUG_SMOOTH]skip_comp[%d] = true\n", i);
					ts->drag_hist[i][SHTPS_POSTYPE_X].skip_comp = true;
					ts->drag_hist[i][SHTPS_POSTYPE_Y].skip_comp = true;
					ts->drag_hist[i][SHTPS_POSTYPE_X].real_gap_FIXED = x_gap_fixed[i];
					ts->drag_hist[i][SHTPS_POSTYPE_Y].real_gap_FIXED = y_gap_fixed[i];
				}else{
					ts->drag_hist[i][SHTPS_POSTYPE_X].skip_comp = false;
					ts->drag_hist[i][SHTPS_POSTYPE_Y].skip_comp = false;
				}
			}
		}
		...
		if(SHTPS_DRAG_SMOOTH_USE_HULL){
			for(i = 0;i < fingerMax;i++){
				...
				if (diagonal_fixed[i] > SHTPS_DRAG_SMOOTH_INT_TO_FIXED(SHTPS_DRAG_SMOOTH_LEAVE_MAX_DOT)) {
					info->fingers[i].x = SHTPS_DRAG_SMOOTH_FIXED_TO_INT(before_x_fixed[i] + ((after_x_fixed[i] - before_x_fixed[i]) /
						...
						SHTPS_DRAG_SMOOTH_LEAVE_MAX_DOT));
	...
	if(SHTPS_DRAG_SMOOTH_FLATTERY){
		...
		for(i = 0;i < fingerMax;i++){
			...
			if(outPointer.toolType != SHTPS_TOUCH_STATE_NO_TOUCH){
				...
				pOriginalHistory_x = &ts->flatteryOriginalHistory[SHTPS_POSTYPE_X][i];
				pOriginalHistory_y = &ts->flatteryOriginalHistory[SHTPS_POSTYPE_Y][i];
				...
				if(shtps_is_low_frame_rate_mode(ts) == 0){
					...
					flattery_exclude_dot = SHTPS_DRAG_SMOOTH_FLATTERY_EXCLUDE_DOT_LOW_FPS;
				}
				...
				ts->flatteryOriginalHistory[SHTPS_POSTYPE_X][i].cnt_available_data = 0;
				ts->flatteryOriginalHistory[SHTPS_POSTYPE_Y][i].cnt_available_data = 0;
			}
